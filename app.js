  ...
  victory: {
    ...
    layouts: {
      landscape: {
        ...
        nicknameAlign: "center",
        clubAlign: "center",
        scoreBox: { x: 118, y: 802, w: 844, h: 176 },
        positions: {
          total: { x: 540 },
          nickname: { x: 540, y: 770 },
          club: { x: 540, y: 1020 },
          extra: { x: 540, y: 1055 }
        }
      },
      portrait: {
        ...
        nicknameAlign: "center",
        clubAlign: "center",
        scoreBox: { x: 118, y: 1088, w: 844, h: 192 },
        positions: {
          total: { x: 540 },
          nickname: { x: 540, y: 1044 },
          club: { x: 540, y: 1330 },
          extra: { x: 540, y: 1372 }
        }
      }
...
function drawBackgroundImage(context) {
  ...
  if (activePosterTemplate === "victory") {
    filters.push("brightness(0.72)", "saturate(0.82)", "contrast(1.12)");
...
function drawBrand(context) {
  ...
  if (isVictory) {
    ...
    context.lineWidth = 2;
    context.beginPath();
    context.moveTo(68, 38);
    context.lineTo(posterSize / 2 - width / 2 - 30, 38);
    context.moveTo(posterSize / 2 + width / 2 + 30, 38);
    context.lineTo(posterSize - 68, 38);
    context.stroke();
    context.fillStyle = "#e23b46";
    context.fillRect(38, 34, 18, 8);
    context.fillRect(posterSize - 56, 34, 18, 8);
    return;
  }
...
function drawTotalScore(context) {
  ...
  if (activePosterTemplate === "victory") {
    ...
    context.strokeStyle = "rgba(42, 0, 4, 0.46)";
    context.lineWidth = Math.max(3, fontSize * 0.01);
...
function drawSubjectOverlay(context) {
  ...
  if (activePosterTemplate === "victory") {
    // A second mask pass suppresses bright low-confidence halos over the darkened original.
    drawCoverAsset(layerCtx, subjectMaskSource, 0, topBarHeight, posterSize, getPosterHeight() - topBarHeight);
    context.save();
...
function drawLabels(context) {
  ...
  const serifItalic = activePosterTemplate === "victory";

  context.save();
  context.shadowColor = "rgba(0,0,0,0.44)";
  context.shadowBlur = activePosterTemplate === "victory" ? 14 : 10;
  context.shadowOffsetY = activePosterTemplate === "victory" ? 4 : 2;
...
function drawScoreCard(context) {
  ...
  if (!gridStyle) {
    if (activePosterTemplate === "victory") {
      ...
      context.fillStyle = elements.scoreTableColor.value;
      context.fillRect(scoreBox.x, scoreBox.y, scoreBox.w, scoreBox.h);
      const scoreShade = context.createLinearGradient(
        scoreBox.x,
        scoreBox.y,
        scoreBox.x + scoreBox.w,
        scoreBox.y
      );
      scoreShade.addColorStop(0, "rgba(255,255,255,0.08)");
      scoreShade.addColorStop(0.48, "rgba(255,255,255,0)");
      scoreShade.addColorStop(1, "rgba(20,0,2,0.28)");
      context.fillStyle = scoreShade;
      context.fillRect(scoreBox.x, scoreBox.y, scoreBox.w, scoreBox.h);
      context.strokeStyle = "rgba(255,255,255,0.22)";
      context.lineWidth = 2;
      context.strokeRect(scoreBox.x + 1, scoreBox.y + 1, scoreBox.w - 2, scoreBox.h - 2);
    } else {
  ...
  const highlightColor = activePosterTemplate === "editorial"
    ? "#ef4d57"
    : activePosterTemplate === "victory"
      ? "#ffffff"
      : "#ffd100";
...
function drawTemplateAtmosphere(context) {
  ...
  if (activePosterTemplate === "victory") {
    const topShade = context.createLinearGradient(0, topBarHeight, 0, posterHeight * 0.38);
    topShade.addColorStop(0, "rgba(0,0,0,0.46)");
    topShade.addColorStop(1, "rgba(0,0,0,0)");
    context.fillStyle = topShade;
    context.fillRect(0, topBarHeight, posterSize, posterHeight * 0.38 - topBarHeight);

    const bottomShade = context.createLinearGradient(0, posterHeight * 0.44, 0, posterHeight);
    bottomShade.addColorStop(0, "rgba(0,0,0,0)");
    bottomShade.addColorStop(0.74, "rgba(0,0,0,0.42)");
    bottomShade.addColorStop(1, "rgba(0,0,0,0.86)");
    context.fillStyle = bottomShade;
    context.fillRect(0, posterHeight * 0.44, posterSize, posterHeight * 0.56);

    const vignetteRadius = Math.max(posterSize, posterHeight) * 0.72;
    ...
    context.fillStyle = "#e23b46";
    context.fillRect(46, topBarHeight + 42, 7, 96);
    context.fillStyle = "rgba(255,255,255,0.52)";
    context.fillRect(70, topBarHeight + 42, 178, 2);
    context.fillStyle = "rgba(255,255,255,0.2)";
    context.fillRect(70, topBarHeight + 54, 88, 2);
    return;
  }

  const startRatio = activePosterTemplate === "editorial" ? 0.38 : 0.56;
  const endAlpha = activePosterTemplate === "editorial" ? 0.58 : 0.44;
...
function renderPoster({ exporting = false } = {}) {
  ...
  ctx.strokeStyle = activePosterTemplate === "victory"
  ...
  if (activePosterTemplate === "victory") {
    ...
    ctx.fillRect(26, 2, 148, 6);
    ctx.fillRect(posterSize - 174, posterHeight - 8, 148, 6);
  }
  if (!exporting) {
